...
namespace yolov5
{
    ...
    void Yolov5PrePostProcessor::ParseModelIO(std::shared_ptr<ov::Model> ov_model)
    {
    ...
    std::vector<Common::BoundingBox> Yolov5PrePostProcessor::ParseRawOutput(ov::Tensor &output_tensor, float conf_threshold)
    {   
        ...
        {
            pYolov5RawOutput bbox = reinterpret_cast<pYolov5RawOutput>(&(*iter));
            if(bbox->box_conf < conf_threshold)
                continue;

            auto cls_conf_start = iter + Yolov5RawOutputEleNum;
    ...
    void Yolov5PrePostProcessor::NMS(std::vector<Common::BoundingBox> &boxes, float iou_threshold)
    {
        boxes = Common::NMS(boxes, iou_threshold);
    }

    void Yolov5PrePostProcessor::RestoreCoords(std::vector<Common::BoundingBox> &boxes, cv::Size original_shape, cv::Size current_shape)
    {
        for(auto& box : boxes) 
        {
